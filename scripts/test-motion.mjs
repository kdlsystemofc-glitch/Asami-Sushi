...
// ═══ 9. SANIDADE (sem fixtures) ═══
{
  ...
  for (const [w, h] of [[1440, 900], [390, 844]]) {
    ...
    const r = await s.page.evaluate(() => ({
      ...
      fora: [...document.querySelectorAll("[data-reveal], [data-parallax], [data-loop]")].filter((el) => !el.closest("#ato-1, #ato-2")).length,
      nigiri: getComputedStyle(document.querySelector(".hero__nigiri")).opacity,
    }));
    ok(r.sw <= r.cw, `${w}px: sem rolagem horizontal (${r.sw} ≤ ${r.cw})`);
    ok(r.fora === 0 && r.nigiri === "1", `${w}px: motion só no hero e no ACT II (${r.fora} data-* fora deles), nigiri visível`);

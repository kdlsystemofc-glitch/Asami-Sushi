...

{
  ...
  for (const [rotulo, query, w, h, semOnda] of casos) {
    ...
    if (semOnda) await page.evaluate(() => gsap.globalTimeline.getChildren(true, true, false).filter((t) => t.vars.repeat === -1).forEach((t) => t.pause()));
...
// ═══ 5. MODOS E QUALIDADE ═══
{
  ...
  // reduced: sem loops, sem parallax, sem entrada, wordmark visível
  {
    ...
    const r = await page.evaluate(() => ({
      ...
      onda: gsap.globalTimeline.getChildren(true, true, false).filter((t) => t.vars.repeat === -1).length,
    }));
  ...
  // paused: tudo congela e retoma
  {
    ...
    const fora = await infinitas(page);
    const ondaFora = await page.evaluate(() => gsap.globalTimeline.getChildren(true, true, false).filter((t) => t.vars.repeat === -1).every((t) => t.paused()));
